#!/bin/bash
#SBATCH --job-name=pyscenic_filtered
#SBATCH --output=pyscenic_logs/pyscenic_filtered_%A_%a.out
#SBATCH --error=pyscenic_logs/pyscenic_filtered_%A_%a.err
...
# Output — separate directory so the original run is preserved
OUTPUT_DIR="pyscenic_filtered_output"
...
echo "============================================"
echo "pySCENIC Pipeline — DB-Gene-Filtered Version"
...
echo "============================================"
echo ""
echo "This version filters the expression matrix to only"
echo "genes present in the cisTarget ranking databases"
echo "BEFORE running GRNBoost2, ensuring ~100% DB overlap"
echo "for all modules and preventing TF pruning due to"
echo "missing Gm-predicted / RIKEN clone genes."
echo "============================================"

${PYTHON_BIN} run_pyscenic_filtered.py \
    --h5ad "${H5AD}" \
